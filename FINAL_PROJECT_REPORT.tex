...
We loosely borrowed the spirit of the \href{https://www.darpa.mil/work-with-us/heilmeier-catechism}{Heilmeier Catechism}---basically, if a busy TA skims this, what should they walk away believing? The pain point is simple: you have a huge attendee list and a vague goal (\emph{I want to meet people in robotics / LLMs / whatever}), but there is no obvious button that turns that into a short ranked list with reasons you can trust. Today that usually means spreadsheets, DMs, or a generic chatbot that is not tied to your actual roster. Our pitch is smaller but concrete: combine hybrid retrieval over 10k+ profile rows with a \textbf{local} LLM that only comments on text we actually retrieved, so we are less likely to ``invent'' a person or a fact. The main caveats are honest ones for a class build---our GTC table is synthetic, not live registration data, and running Llama on CPU can feel slow, so we lean on caching and a thin generation step.
